...
int main(int argc, char** argv) {
  ...
  if(argc != 5 && argc != 4) 
  {
      ...
      // making rand have a max of 252 ensures matrices will have less than 64000 elements
      // M.height * N.width = range(0,252) * range(0,252) < 64000
      // this also just makes it easier to test because colab wouldnt show me the whole output when the matrix was huge
      //      which means I couldn't see how many elements didn't match
      M = AllocateMatrix(rand() % 252, rand() % 1024, 1);
      N = AllocateMatrix(M.width, rand() % 252, 1);
      // M  = AllocateMatrix(145, 251, 1);
      // N  = AllocateMatrix(M.width, 217, 1);
...
void MatrixMulOnDevice(const Matrix M, const Matrix N, Matrix P)
{
  ...
  int gridy = max(ceil(M.height/(double)TILE_WIDTH), ceil(N.height/(double)TILE_WIDTH));
  int gridx = max(ceil(M.width/(double)TILE_WIDTH), ceil(N.width/(double)TILE_WIDTH));
